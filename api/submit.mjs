...
export function createHandler({ env = process.env, fetchImpl = globalThis.fetch } = {}) {
  return async function submit(req, res) {
    ...
    try {
      ...
      if (incoming.format !== 'expert-validation/2.4' || incoming.instrumentVersion !== instrument.version) return fail(400, 'Versión de formulario no compatible.');
      ...
      state.step = instrument.dimensions.length + 4; state.focusId = null; state.submission = null;
