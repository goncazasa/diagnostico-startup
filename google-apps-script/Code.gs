...
function doPost(e) {
  ...
  try {
    ...
    const supportedInstrument =
      (payload.format === 'expert-validation/2.1' && payload.instrumentVersion === '2.1.0') ||
      ...
      (payload.format === 'expert-validation/2.7' && payload.instrumentVersion === '2.7.0') ||
      (payload.format === 'expert-validation/2.8' && payload.instrumentVersion === '2.8.0') ||
      (payload.format === 'expert-validation/2.9' && payload.instrumentVersion === '2.9.0');
...
function summaryRows_(latest, instruments) {
  const entries = Object.keys(latest).map(key => latest[key].payload).filter(payload => payload.instrumentVersion === '2.9.0');
  ...
  const instrument = instruments['2.9.0'];
  const rows = [
...
function contentValidity_(latest, instruments) {
  ...
  Object.keys(instruments).filter(version => version === '2.9.0').forEach(version => {
