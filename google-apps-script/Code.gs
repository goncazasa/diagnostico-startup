...
function summaryRows_(latest, instruments) {
  const entries = Object.keys(latest).map(key => latest[key].payload).filter(payload => payload.instrumentVersion === '2.3.0');
  ...
  const instrument = instruments['2.3.0'];
  const rows = [
...
function contentValidity_(latest, instruments) {
  ...
  Object.keys(instruments).filter(version => version === '2.3.0').forEach(version => {
