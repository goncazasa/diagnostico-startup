...
function boot(saved, denyStorage = false) {
  ...
  const dom = new JSDOM(html, { url: 'https://validation.test/', runScripts: 'dangerously', beforeParse(window) {
    ...
    if (saved) window.localStorage.setItem('startup-expert-validation-v2.9', saved);
  ...
  }, click(selector) { const node = document.querySelector(selector); assert.ok(node, selector); node.click(); }, stored() { return JSON.parse(dom.window.localStorage.getItem('startup-expert-validation-v2.9')); } };
}
function start(ui) {
...
test('backup clipboard export contains pending answers', async () => {
  ...
  assert.equal(payload.instrumentVersion, '2.9.0');
...
test('a competing tab cannot be silently overwritten', () => {
  ...
  ui.dom.window.dispatchEvent(new ui.dom.window.StorageEvent('storage', { key: 'startup-expert-validation-v2.9', newValue: '{"other":"response"}' }));
...
test('each dimension shows all its questions and continues to the next dimension', () => {
  ...
  ui.click('[data-action="next"]');
  assert.equal(ui.document.querySelectorAll('#app article.item').length, 8);
...
test('sidebar counters count questions only and exclude the dimension-wide rating',()=>{
 ...
 assert.deepEqual(counts(),['0/5','0/8','0/3','0/4','0/2']);
 ui.input('input[data-path="items.E1.relevance"][value="3"]',true);
 assert.deepEqual(counts(),['1/5','0/8','0/3','0/4','0/2']);
