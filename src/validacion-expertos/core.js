(function (root) {
  'use strict';
  const SCHEMA = 'expert-validation/2.7';
  ...
  function createModel(instrument) {
    ...
    function validateState(candidate) {
      ...
      check(candidate.schemaVersion === SCHEMA && candidate.instrumentVersion === instrument.version, 'Este archivo pertenece a otra versión. Usa una copia exportada desde V2.7.');
    ...
    function importPayload(payload) {
      check(plain(payload) && payload.format === SCHEMA && payload.instrumentVersion === instrument.version, 'Selecciona una copia de V2.7. Las versiones anteriores tienen preguntas o criterios distintos y deben abrirse en su formulario original.');
