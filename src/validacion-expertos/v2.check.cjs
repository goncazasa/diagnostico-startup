...
const ids=['E1','E2','E3','E4','E5','AM1','AM2','AM3','AM4','AM5','AM6','AM7','AM8','MV1','MV2','MV3','GF1','GF2','GF3','GF4','GR1','GR2'];
...
questions.splice(9,0,'¿Existe evidencia de que las condiciones actuales del mercado hacen que este sea un buen momento para impulsar el proyecto?');

test('V2.9 keeps an ordered five-dimensional bank of 22 universal questions',()=>{
 assert.equal(instrument.version,'2.9.0');assert.equal(instrument.schema,'expert-validation/2.9');
 ...
 assert.deepEqual(instrument.dimensions.map(d=>instrument.items.filter(i=>i.dim===d.id).length),[5,8,3,4,2]);
});
test('V2.9 uses four evidence levels and no conditional question route',()=>{
...
test('V2.9 has no prohibited scored topic',()=>{
...
test('V2.9 calculations require all 27 relevance blocks and isolate earlier imports',()=>{
 let s=model.createState('v29-check');s.consent=true;s.profile.email='expert@example.org';s=model.sealInitial(s);assert.equal(model.deliveryIssues(s).length,27);
 for(const a of [...Object.values(s.items),...Object.values(s.dimensions)])a.relevance=3;assert.deepEqual(model.deliveryIssues(s),[]);
 const payload=model.exportPayload(s);assert.equal(payload.format,'expert-validation/2.9');payload.format='expert-validation/2.8';assert.throws(()=>model.importPayload(payload),/V2\.9/);
});
test('V2.9 renders the revised descriptions, guidance and response anchors',()=>{
 ...
 assert.match(items.E3.note,/responsabilidad que tendrá/i);
 ...
 assert.match(items.AM5.q,/buen momento/i);
 assert.doesNotMatch(items.AM5.q,/monitoriza/i);
 assert.match(items.AM6.q,/monitorizan los cambios/i);
