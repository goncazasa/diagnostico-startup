...
test('testing records are labelled, excluded from academic validity and summarized separately',()=>{
 ...
 const comments=x.tables.get('Comentarios');assert.ok(comments);assert.ok(comments[0].includes('campo'));assert.ok(comments.some(r=>r.includes('DATOS DE TESTING · Ana'))===false);
 const methods=x.tables.get('Metodologia');assert.ok(methods);assert.ok(methods.some(r=>r[0]==='I-CVI'));assert.ok(methods.some(r=>r[0]==='Kappa modificado'));
});
...
test('historical V1.9 archives stay intact and separate when rebuilding V2 tables',()=>{
 ...
 legacy.response.dimensions=Object.fromEntries(legacy.instrument.dimensions.map(d=>[d.id,{relevance:3,coverage:null,skipReason:'',comment:''}]));
 legacy.response.items=Object.fromEntries(legacy.instrument.items.map(i=>[i.id,{relevance:3,usability:null,skipReason:'',comment:i.id==='T3'?'Dato histórico V1.9':''}]));
 ...
 const ratings=x.tables.get('Valoraciones');assert.equal(ratings.filter(r=>r[1]==='1.9.0').length,27);assert.equal(ratings.filter(r=>r[1]==='2.8.0').length,26);
